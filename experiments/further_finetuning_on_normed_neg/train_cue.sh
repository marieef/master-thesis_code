...
DIR_NAME='FINETUNE_nbbert-large_3456_CUE' # must exist inside DIR_PATH_PREFIX
BATCH_SIZE=16
...
for seed in ${SEEDS[@]} ; do
	for train_set in ${TRAIN_SETS[@]} ; do
		sbatch train_cue.slurm --dir_path_prefix $DIR_PATH_PREFIX --batch_size $BATCH_SIZE --epochs $EPOCHS --initial_lr 0.00003 --warmup_steps 200 --model_path NbAiLab/nb-bert-large --train_set $train_set --dev_set ../../normed_neg_data/datasplit/NorMed_neg_dev.sem --dir_name $DIR_NAME --seed $seed --trained_model_path $TRAINED_MODEL_PATH
	done
done
