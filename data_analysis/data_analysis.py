"""From the NoReC_neg repo: https://github.com/ltgoslo/norec_neg/blob/main/data/data_analysis.py"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filename_prefix", default="NorMed_neg_")
    ...
    neg_sent_lengths = {"train": [], "dev": [], "test": []} # Added by me to get statistics for _negated_ sents as well
    ...
    for split in ["train", "dev", "test"]:
        with open(os.path.join(args.data_dir, args.filename_prefix + split + ".json"), encoding="utf8") as infile:
        ...
        for sentence in data:
            ...
            if len(sentence["negations"]) > 0:
                ...
                neg_sent_lengths[split].append(len(sentence["text"].split()))
    ...
    for split in ["train", "dev", "test"]:
        ...
        print("---- negated sents, max len: {0}".format(np.max(neg_sent_lengths[split])))
        print("---- negated sents, ave len: {0:.1f}".format(np.mean(neg_sent_lengths[split])))
    ...
    all_neg_sent_lengths = [i for k in neg_sent_lengths.values() for i in k]
    ...
    print("---- negated sents, max len: {0}".format(np.max(all_neg_sent_lengths)))
    print("---- negated sents, ave len: {0:.1f}".format(np.mean(all_neg_sent_lengths)))
    print()
