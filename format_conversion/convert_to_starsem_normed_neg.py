# This file is equal to https://github.com/ltgoslo/norec_neg/blob/main/modeling/evaluation/convert_to_starsem.py, 
# except that we added all cues occurring in NorMed_neg to the prefixes, suffixes and no_affix lists, in order to 
# be able to convert the dataset correctly from json to starsem format.  
import re
import json
import argparse
import numpy as np
...
prefixes = ["ab", "an", "a", "A", "ikke-", "ikke", "im", "in", "mis", "utenom", "U", "u"]
suffixes = ["frie", "fritt", "fri", "løse", "løst", "løs", "tomme"]
no_affix = ['Unntaket', 'fri', 'inga', 'ingen ting', 'ingenting', 'ingen', 'inget',
            'intet', 'mister', 'mistet', 'unngår', 'unngå', 'unntaket', 'unntak',
            'unntatt', 'uteble', 'uteblir',
            "Ingen", "utelates", "utelatt", "utelukkes", "utelukket", "utelukke", "uten", 'Uten', 'utan',
            'aldri']
...
def json_file_to_starsem(fn_in: str, fn_out: str) -> None:
    with open(fn_in, encoding='utf8') as fh_in, open(fn_out, "w", encoding='utf8') as fh_out:
